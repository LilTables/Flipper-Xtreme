...
static void bad_usb_draw_callback(Canvas* canvas, void* _model) {
    ...
    if((model->state.state == BadUsbStateIdle) || (model->state.state == BadUsbStateDone) ||
       (model->state.state == BadUsbStateNotConnected)) {
        elements_button_center(canvas, "Launch");
    ...
    if(model->state.state == BadUsbStateNotConnected) {
        ...
        canvas_draw_str_aligned(canvas, 127, 43, AlignRight, AlignBottom, "in!");
    } else if(model->state.state == BadUsbStateWillRun) {
        canvas_draw_icon(canvas, 4, 26, &I_Clock_18x18);
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str_aligned(canvas, 127, 31, AlignRight, AlignBottom, "Will launch");
